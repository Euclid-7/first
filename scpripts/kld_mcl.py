...
class KldMcl(Mcl):
    ...
    def motion_update(self,nu,omega,time):
        ...
        for i in range(self.max_num):
            ...
            self.binnnum=len(bins) if len(bins) > 1 else 2
...
def trial():
    ...
    ##地図を生成して2つランドマークを追加##           #一つランドマークを減らしておく
    m=Map()
...
#     self.resampling()             #motion_updateでリサンプリングするので削除
    ...
    def draw(self, ax, elems):  
        super().draw(ax, elems)
        elems.append(ax.text(-4.5, -4.5, "paricle:{}, bin:{}".format(len(self.particles), self.binnum), fontsize=10)) #パーティクルと


# In[12]:


def trial():  ###kldmcl1exec
    time_interval = 0.1
    world = World(30, time_interval, debug=False) 
