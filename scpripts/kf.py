...
class KalmanFilter:   ###kfmotionupdate
    ...
    def observation_update(self,observation): 
        for d in observation:
            ...
            belief_suggest_pos=IdealCamera.relative_polar_pos(self.pose,pos_on_map)
            
            distance_dev=distance_dev_rate*belief_suggest_pos[0]
            cov=np.diag(np.array([distance_dev**2,direction_dev**2]))
            self.belief*=multivariate_normal(mean=belief_suggest_pos,cov=cov).pdf(z)
